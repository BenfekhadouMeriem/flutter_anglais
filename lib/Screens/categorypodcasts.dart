  ...
  Future<void> fetchCategoryPodcasts() async {
    ...
    try {
      ...
      final querySnapshot = await _firestore
          .collection('contents')
          .where('categoryId', isEqualTo: _firestore.doc('categories/${widget.categoryId}'))
          .get();
      print("📋 Réponse Firestore : ${querySnapshot.docs.length} podcasts reçus");
      ...
      final convertedData = querySnapshot.docs.map((doc) {
        ...
        return {
          ...
          'file_path': _convertGoogleDriveUrl(data['audioUrl']?.toString() ?? ''),
          'image_path': _convertGoogleDriveUrl(data['imageUrl']?.toString() ?? ''),
